    ...
    unsigned int xid_rep, char **b, int *sz)
{
  ...
  if (reply_window_.find(clt_nonce) == reply_window_.end()) {
    ...
    auto clt = reply_window_[clt_nonce];
    //for_each(clt.begin(),clt.end(),
    //see this xid
    auto it = clt.begin();
    ...
    if(ret == DONE || ret == INPROGRESS) {
      return ret;
    }
    else{
      auto itfirstreply = clt.begin();
      if (clt.size() != 0 && xid < itfirstreply->xid) {
        return FORGOTTEN;
      }
      else {
        return NEW;
    ...
    char *b, int sz)
{
  ...
  reply_t replyv(xid);
  replyv.sz = sz;
  replyv.buf = (char *)malloc(sz);
  VERIFY(replyv.buf);
  memcpy(replyv.buf,b,sz);
  replyv.cb_present = true;
